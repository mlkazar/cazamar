...
@implementation RadioListHistory {
    /* get data from topEdit's histItems */
    RadioHistory *_topHist;
...
- (RadioListHistory *) initWithParent:(RadioHistory *) hist tableView: (UITableView *) tview
{
    self = [super init];
    if (self) {
	_topHist = hist;
...
- (UITableViewCell *) tableView: (UITableView *) tview cellForRowAtIndexPath: (NSIndexPath *)path
{
    ...
    if (hist.highlighted) {
	cell.backgroundColor = [UIColor colorWithRed: 0.6
					       green: 0.8
						blue:1.0
					       alpha: 1.0];
    } else {
...
@implementation RadioHistory {
    ...
    ViewController *_viewCont;	/* back ptr to our view controller */
    RadioListHistory *_listHist;		/* to supply data to _listTableView */
...
- (id)initWithViewController: (ViewController *) viewCont
{
	...
	_listHist = [[RadioListHistory alloc] initWithParent: self tableView: _listTableView];
...
- (void) clearPressed: (id) sender withData: (NSNumber *) number
{
    ...
    [_viewCont presentViewController: alert animated:YES completion: nil];
}
...
- (void) restoreEdits
{
    ...
    if (inFilep == NULL) {
	NSLog(@"RadioHistory: failed to open history.txt");
	return;
    }
...
- (void) saveEdits
{
    ...
    if (code != nbytes) {
	NSLog(@"RadioHistory: failed to write all data %d should be %d", code, nbytes);
	[writer cleanup];
    } else {
...
- (void) addHistoryStation: (NSString *) station withSong: (NSString *) song
{
    ...
    unknownString = MFANStreamPlayer_getUnknownString();
