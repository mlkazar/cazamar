 ...
 */
int32_t
Json::parseJsonPair(InStream *streamp, Node **nodepp, int32_t level)
{
    std::string token;
    std::string key;
    int isSingle;
    ...
    if (!isSingle) {
        ...
        code = parseJsonValue(streamp, &nodep, level);
        if (code)
            return code;
...
int32_t
Json::parseJsonValue(InStream *streamp, Node **nodepp, int32_t indent)
{
    std::string token;
    std::string value;
    int isSingle;
    ...
    if (!isSingle) {
        ...
        code = parseJsonComplex(streamp, &token, nodepp, indent);
        return code;
    }
    else {
...
Json::parseJsonComplex(InStream *streamp, std::string *firstTokenp, Node **nodepp, int32_t indent)
{
    ...
    while(1) {
        printIndentX(indent+1);
        if (isArray)
            code = parseJsonValue(streamp, &childp, indent+1);
        else
            code = parseJsonPair(streamp, &childp, indent+1);
        if (code)
            return code;
...
Json::parseJsonChars(char **inDatapp, Json::Node **nodepp)
{
    ...
    code = parseJsonValue(&inStream, nodepp, 0);
    if (code == 0) {
